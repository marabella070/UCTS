...
#include "universal_converter_to_string.h"  // for InvalidTypeConversionExce...
...
bool test_conversion(T&& initial_value, const std::string& expected_value) {
    std::string result;
    try {
        result = dev_toolkit::ucts::universal_converter_to_string(std::forward<T>(initial_value));

    } catch (const dev_toolkit::ucts::InvalidTypeConversionException& e) {
...
TEST(UniversalConverterTest, IntegerTypes) {
    ...
    EXPECT_TRUE(
        test_conversion(static_cast<unsigned char>('a'), "a"));  //! unsigned char, pay attention, result is "a"!
    ...
    EXPECT_TRUE(
        test_conversion(static_cast<unsigned long long>(123456789012ULL), "123456789012"));  // unsigned long long
}
...
TEST(UniversalConverterTest, FloatingPointTypesExtremes) {
    ...
    EXPECT_TRUE(test_conversion(static_cast<double>(123456789.0),
                                "123456789.000000"));  // large_number - формат без экспоненты, с шестью знаками
    EXPECT_TRUE(test_conversion(static_cast<double>(-987654.321), "-987654.321000"));  // large_negative_number - same
    EXPECT_TRUE(test_conversion(static_cast<double>(3.0000001),
                                "3.000000"));  // little_over_then_inteher - округляется в пределах 6 знаков
    EXPECT_TRUE(test_conversion(static_cast<double>(2.9999999), "3.000000"));  // almost_integer - округление вверх
    EXPECT_TRUE(test_conversion(static_cast<double>(0.0000012345),
                                "0.000001"));  // small_number - обрезается до шести знаков после запятой
...
TEST(UniversalConverterTest, ConstructibleTypes) {
    ...
    std::initializer_list<char> init_list = {'i', 'n', 'i', 't', 'i', 'a', 'l', 'i',
                                             'z', 'e', 'r', '_', 'l', 'i', 's', 't'};
...
TEST(UniversalConverterTest, SpecialConstructibleTypes) {
    ...
    EXPECT_TRUE(test_conversion(WithExplicitConvertOperator(), "explicit_convert_operator"));
...
TEST(UniversalConverterTest, NotSupportedTypes) {
    EXPECT_FALSE(test_conversion(nullptr, "nullptr_t"));  // Interesting extreme case.
    EXPECT_FALSE(test_conversion(static_cast<void*>(nullptr), "void*"));
    EXPECT_FALSE(test_conversion(std::vector<int>{}, "std::vector<int>"));
    EXPECT_FALSE(test_conversion(std::deque<std::string>{}, "std::deque<std::string>"));
}
